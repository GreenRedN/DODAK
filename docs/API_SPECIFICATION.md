...
3. 문장별 감정 키워드 분석
4. 긍정/부정/중립 1차 분류
5. 9가지 감정 분류 (기쁨, 희망, 중립, 분노, 슬픔, 불안, 피로, 후회, 우울)
6. 비율 계산 및 반환
...
| 필드 | 타입 | 설명 |
|------|------|------|
| arrangeIdx | int | 배정 PK |
| id | int | 환자(회원) FK |
| doctIdx | int | 의사 FK |
| createdAt | datetime | 배정 일시 |
...
| 1.2 | 2026-02-09 | 배정 API 정정 (예약→배정), 9가지 감정 반영 | 박제연 |
